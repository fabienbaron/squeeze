...
pro image_cont_uv, a, WINDOW_SCALE = window_scale, ASPECT = aspect, $
	...
	if (keyword_set(nocontours) eq 0) then begin
		...
		if (n_ct gt 0) then contour,a,x,y,/noerase,/xst,/yst,$      ;Do the contour
		pos = [px(0),py(0), px(0)+swx,py(0)+swy],/dev,$
		...
		; FB: do not use contour function at all
; normal axis
if !d.name eq 'PS' then begin
	; draw only title in black
plot, x, y, pos=[px(0),py(0), px(0)+swx,py(0)+swy],  /noerase, /dev, /nodata,tit=tit, xtit=xtit, ytit=ytit, xr=[x(0),x(n_elements(x)-1)], yr =[y(0),y(n_elements(y)-1)],  color = 0
; now draw axes only
plot, x, y, pos=[px(0),py(0), px(0)+swx,py(0)+swy],  /noerase, /dev, /nodata, xr=[x(0),x(n_elements(x)-1)], yr =[y(0),y(n_elements(y)-1)], xticks = 4, xminor = 4, yticks=4, yminor = 4, color = 254,  ytickname=replicate(' ', 5), xtickname=replicate(' ', 5)
endif else begin
	plot, x, y, pos=[px(0),py(0), px(0)+swx,py(0)+swy],  /noerase, /dev, /nodata,tit=tit, xtit=xtit, ytit=ytit, xr=[x(0),x(n_elements(x)-1)], yr =[y(0),y(n_elements(y)-1)]
endelse
