  ...
  if (use_bandwidthsmearing == TRUE)
    {
      for (j = 0; j < axis_len; ++j)
        for (i = 0; i < axis_len; ++i)
            for (k = 0; k < nuv; k++)
              dft_transform[(j*axis_len+i)*nuv+k] = sinc(M_PI * mas_pixel / MAS_RAD * uv_dlambda[k] / uv_lambda[k] * ((i - axis_len / 2) * u[k] - (j - axis_len / 2) * v[k])) *
                cexp(2.0 * M_PI * I * mas_pixel / MAS_RAD * ((i - axis_len / 2) * u[k] - (j - axis_len / 2) * v[k]));
      // Note?: u[k] and v[k] already include a factor 1/lambda
    }
  else
    {
      for (j = 0; j < axis_len; ++j)
        for (i = 0; i < axis_len; ++i)
            for (k = 0; k < nuv; k++)
            dft_transform[(j*axis_len+i)*nuv+k] = cexp(2.0 * M_PI * I * mas_pixel / MAS_RAD * ((i - axis_len / 2) * u[k] - (j - axis_len / 2) * v[k]));
    }
...
#pragma omp barrier
    ...
    //
    compute_model_visibilities_fromelements(mod_vis, im_vis, param_vis, params, fluxratio_image, element_x, element_y, dft_transform,
        ...
        if ((i % (STEPS_PER_OUTPUT * nwavr * nelements)) == 0)
          {
            if (use_tempfitswriting == TRUE)
              writeasfits(temp_filename, image, nwavr, 1, (iChain)*niter + i / (nelements * nwavr), 2.0 * lLikelihood / ndf, chi2v2, chi2t3amp, chi2t3phi,
                          chi2visamp, chi2visphi, temperature[iChain], nelements, &reg_param[0], &reg_value[0], niter, axis_len, ndf, tmin, chi2_temp,
                          chi2_target, mas_pixel, nchains, 0, 0, "", "", &saved_params[iChaintoStorage[iChain] * nparams * niter], NULL);

            // PRINT DIAGNOSTICS
            if (squeeze_quiet == FALSE)
              print_diagnostics(iChain, (i / (nwavr * nelements) + 1), nvis, nv2, nt3, nt3phi, nt3amp, nvisamp, nvisphi, chi2v2, chi2t3amp, chi2t3phi,
                                chi2visphi, chi2visamp, lPosterior, lPrior, lLikelihood, reg_param, reg_value, centroid_image_x, centroid_image_y, nelements,
                                nwavr, niter, temperature, prob_movement, params, stepsize, burn_in_times);
            // getchar();
            if (prob_auto > 0)
            ...
            for (j = 0; j < nuv; ++j)
              {
                if (uvwav2chan[j] == chan)
                    new_im_vis[j] = im_vis[j] + (dft_transform[(new_y*axis_len+new_x)*nuv+j] - dft_transform[(old_y*axis_len+old_x)*nuv+j]) * fluxratio_image[j] / (double)nelements;
                else
            ...
            else // parameter movement was attempted and rejected
              {
                ...
                if (stepsize[params_free[(current_elt - nelements) / PARAMS_PER_ELT]] == 0)
                  printf("Failed parameter movement of 0 stepsize: %d %6.3lf Iter: %ld P: %6.4lf %6.3lf \n",
                         params_free[(current_elt - nelements) / PARAMS_PER_ELT], 2. * (new_lLikelihood - lLikelihood), i, params[0], new_params[0]);
              }
          }
    ...
    if (ctrlcpressed == FALSE)
      {
        if (use_tempfitswriting == TRUE)
          writeasfits(temp_filename, image, nwavr, 1, iChain * niter + i / (nelements * nwavr), 2.0 * lLikelihood / ndf, chi2v2, chi2t3amp, chi2t3phi,
                      ...
                      mas_pixel, nchains, 0, 0, "", "", &saved_params[iChain * niter * nparams], NULL);
      }
      ...
      if (minimization_engine != ENGINE_PARALLEL_TEMPERING) // CASE = (parallel) simulated annealing
        {
          ...
          mcmc_results(minimization_engine, output_filename, nchains, burn_in_times, depth, nelements, axis_len, dft_transform, saved_x, saved_y,
...
void mcmc_writeoutput(char *file_basename, double *image, const int nchains, const int nrealizations, const unsigned int *burn_in_times, const long depth,
                      const long nelements, const unsigned short axis_len,
                      const double complex *__restrict dft_transform, const unsigned short *saved_x, const unsigned short *saved_y, const double *saved_params,
                      ...
                      char *prior_filename, double logZ, double logZe)
{
  ...
  compute_model_visibilities_fromimage(mod_vis, im_vis, param_vis, params, fluxratio_image, image, dft_transform, &lPriorModel, nparams, nelements, axis_len);
...
void mcmc_results(int minimization_engine, char *file_basename, const int nchains, const unsigned int *burn_in_times, const long depth, const long nelements,
                  const unsigned short axis_len, const double complex *__restrict dft_transform,
...
// the depth input should be the actual available depth, not the requested one, unless the chain did not converge
{
      ...
      for (t = 0; t < nchains_eff; ++t)
        {
          ...
          if (nchains_eff < 2)
            {
              ...
              mcmc_writeoutput(data_filename, &images_mean[t * nwavr * axis_len * axis_len], nchains, niter - burn_in_times[t], &burn_in_times[t], depth,
                               nelements, axis_len, dft_transform, saved_x, saved_y, saved_params, niter, nwavr, final_params, final_params_std,
          ...
          mcmc_writeoutput(data_filename, &images_mean[t * nwavr * axis_len * axis_len], nchains, niter - burn_in_times[t], &burn_in_times[t], depth, nelements,
                           axis_len, dft_transform, saved_x, saved_y, saved_params, niter, nwavr, final_params, final_params_std, reg_param,
      ...
      for (t = 0; t < nchains_eff; ++t)
        {
          ...
          mcmc_writeoutput(data_filename, &images_median[t * nwavr * axis_len * axis_len], nchains, niter - burn_in_times[t], &burn_in_times[t], depth,
                           nelements, axis_len, dft_transform, saved_x, saved_y, saved_params, niter, nwavr, final_params, final_params_std, reg_param,
                           final_reg_value, prior_image, initial_x, initial_y, centroid_image_x, centroid_image_y, fov, cent_mult, ndf, tmin, chi2_temp,
                           chi2_target, mas_pixel, init_filename, prior_filename, logZ, logZe);
        }
    }
  ...
  // mcmc_writeoutput(file_basename, image, nchains, nburned, burn_in_times, depth, nelements, axis_len, dft_transform,
...
void compute_model_visibilities_fromelements(double complex *mod_vis, double complex *im_vis, double complex *param_vis, double *params,
                                             ...
                                             const double complex *dft_transform, double *lPriorModel, long nparams, long nelements, short axis_len)
{
  long i, u;
  if (nparams > 0)
  ...
  for (u = 0; u < nuv; ++u)
    {
      im_vis[u] = 0;
      for (i = 0; i < nelements; ++i)
        im_vis[u] += dft_transform[(element_y[uvwav2chan[u] * nelements + i]*axis_len+element_x[uvwav2chan[u] * nelements + i])*nuv+u];
      im_vis[u] *= fluxratio_image[u] / (double)nelements; // Will add SED here
      mod_vis[u] = param_vis[u] + im_vis[u];
    }
}
...
void compute_model_visibilities_fromimage(double complex *mod_vis, double complex *im_vis, double complex *param_vis, const double *params,
                                          double *fluxratio_image, const double *image, const double complex *dft_transform,
  ...
  for (u = 0; u < nuv; ++u)
    {
      ...
      for (ix = 0; ix < axis_len; ix++)
        for (iy = 0; iy < axis_len; iy++)
          im_vis[u] += image[uvwav2chan[u] * axis_len * axis_len + iy * axis_len + ix] * dft_transform[(iy*axis_len+ix)*nuv+u];
    }
      ...
      else if (*argc > i + 1) /* Now the options with arguments*/
        {
          if (strcmp(argv[i], "-s") == 0)
          ...
          else if (strcmp(argv[i], "-S") == 0)
            {
              ...
              if (nparams < 1)
                printf("Error, must call -P option before -S\n");
