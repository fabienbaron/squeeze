...
long nuv;
char *oifits_file;
long nvis, nv2, nt3, nt3phi, nt3amp, nt3amp_orphans, nt3phi_orphans, nt4, nt4phi, nt4amp, nt4amp_orphans, nt4phi_orphans, nvisamp, nvisphi, nvisamp_orphans,
    nvisphi_orphans;
...
#pragma omp barrier
    ...
    if (squeeze_quiet == FALSE)
      print_diagnostics(iChain, -1, nvis, nv2, nt3, nt4, nvisamp, nt3amp, nt4amp, nvisphi, nt3phi, nt4phi, chi2visamp, chi2v2, chi2t3amp, chi2t4amp, chi2visphi,
                        chi2t3phi, chi2t4phi, lPosterior, lPrior, lLikelihood, reg_param, reg_value, centroid_image_x, centroid_image_y, nelements, nwavr,
                              ...
                              if (log(RngStream_RandU01(rng)) < transition_test)
                                {
                                  ...
                                  //                   for(j=0;j<nchains;j++)
                                  // printf("BEFORE Chain %ld \t iChaintoStorage %d \t iStoragetoChain %d \t temp[j] %lf \t temp[iStorage[j]] %lf \t
                                  // temp[iChain[j] %lf\n", j, iChaintoStorage[j], iStoragetoChain[j], temperature[j],
                                  // temperature[iStoragetoChain[j]],temperature[iChaintoStorage[j]] );
                                  ...
                                  // for(j=0;j<nchains;j++)
                                  //  printf("AFTER Chain %ld \t iChaintoStorage %d \t iStoragetoChain %d \t temp[j] %lf \t temp[iStorage[j]] %lf \t
                                  //  temp[iChain[j] %lf\n", j, iChaintoStorage[j], iStoragetoChain[j], temperature[j],
                                  //  temperature[iStoragetoChain[j]],temperature[iChaintoStorage[j]] );
...
#pragma omp barrier // re-synchronise Chains before continuing
        ...
        if ((i % (STEPS_PER_OUTPUT * nwavr * nelements)) == 0)
          {
            if (use_tempfitswriting == TRUE)
              writeasfits(temp_filename, image, nwavr, 1, (iChain)*niter + i / (nelements * nwavr), 2.0 * lLikelihood / ndf, chi2visamp, chi2v2, chi2t3amp,
                          chi2t4amp, chi2visphi, chi2t3phi, chi2t4phi, temperature[iChain], nelements, &reg_param[0], &reg_value[0], niter, axis_len, ndf, tmin,
                          chi2_temp, chi2_target, mas_pixel, nchains, 0, 0, "", "", &saved_params[iChaintoStorage[iChain] * nparams * niter], NULL);

            // PRINT DIAGNOSTICS
            if (squeeze_quiet == FALSE)
              print_diagnostics(iChain, (i / (nwavr * nelements) + 1), nvis, nv2, nt3, nt4, nvisamp, nt3amp, nt4amp, nvisphi, nt3phi, nt4phi, chi2visamp,
                                chi2v2, chi2t3amp, chi2t4amp, chi2visphi, chi2t3phi, chi2t4phi, lPosterior, lPrior, lLikelihood, reg_param, reg_value,
                                centroid_image_x, centroid_image_y, nelements, nwavr, niter, temperature, prob_movement, params, stepsize, burn_in_times);
            // getchar();
            if (prob_auto > 0)
        ...
        if ((nparams == 0) || (current_elt < nelements)) /* Attempt image movement rather than parametric model movement */
          {
            ...
            if (reg_param[REG_L0CDF53] > 0.0)
              new_reg_value[chan * NREGULS + REG_L0CDF53] =
                  L0_CDF53(&image[chan * axis_len * axis_len], NULL, 0.0, axis_len, axis_len, (const double)nelements);

            if (reg_param[REG_L1CDF53] > 0.0)
              new_reg_value[chan * NREGULS + REG_L1CDF53] =
                  L1_CDF53(&image[chan * axis_len * axis_len], NULL, 0.0, axis_len, axis_len, (const double)nelements);

            if (reg_param[REG_L0CDF97] > 0.0)
              new_reg_value[chan * NREGULS + REG_L0CDF97] =
                  L0_CDF97(&image[chan * axis_len * axis_len], NULL, 0.0, axis_len, axis_len, (const double)nelements);

            if (reg_param[REG_L1CDF97] > 0.0)
              new_reg_value[chan * NREGULS + REG_L1CDF97] =
                  L1_CDF97(&image[chan * axis_len * axis_len], NULL, 0.0, axis_len, axis_len, (const double)nelements);

            if (reg_param[REG_L0ATROUS] > 0.0)
              new_reg_value[chan * NREGULS + REG_L0ATROUS] =
                  L0_ATROUS(&image[chan * axis_len * axis_len], NULL, 0.0, axis_len, axis_len, (const double)nelements);
        ...
        compute_lLikelihood(&new_lLikelihood, new_mod_vis, res, mod_obs, &chi2visamp, &chi2v2, &chi2t3amp, &chi2t4amp, &chi2visphi, &chi2t3phi, &chi2t4phi,
                            nwavr);
        compute_lPrior_allwav(&lPrior, nwavr, reg_param, reg_value);
    ...
    if (ctrlcpressed == FALSE)
      {
        if (use_tempfitswriting == TRUE)
          writeasfits(temp_filename, image, nwavr, 1, iChain * niter + i / (nelements * nwavr), 2.0 * lLikelihood / ndf, chi2visamp, chi2v2, chi2t3amp,
                      chi2t4amp, chi2visphi, chi2t3phi, chi2t4phi, temperature[iChain], nelements, &reg_param[0], &reg_value[0], niter, axis_len, ndf, tmin,
                      chi2_temp, chi2_target, mas_pixel, nchains, 0, 0, "", "", &saved_params[iChain * niter * nparams], NULL);
      }
      ...
      if (minimization_engine != ENGINE_PARALLEL_TEMPERING) // CASE = (parallel) simulated annealing
        {
          ...
          mcmc_results(minimization_engine, output_filename, nchains, burn_in_times, depth, nelements, axis_len, xtransform, ytransform, saved_x, saved_y,
                       saved_params, niter, nwavr, final_params, final_params_std, reg_param, final_reg_value, prior_image, initial_x, initial_y,
                       centroid_image_x, centroid_image_y, fov, cent_mult, ndf, tmin, chi2_temp, chi2_target, mas_pixel, init_filename, prior_filename, 0, 0);
        }
      else // (minimization_engine == ENGINE_PARALLEL_TEMPERING)
        {
          ...
          mcmc_results(minimization_engine, output_filename, nchains, burn_in_times, depth, nelements, axis_len, xtransform, ytransform, saved_x, saved_y,
                       saved_params, niter, nwavr, final_params, final_params_std, reg_param, final_reg_value, prior_image, initial_x, initial_y,
                       centroid_image_x, centroid_image_y, fov, cent_mult, ndf, tmin, chi2_temp, chi2_target, mas_pixel, init_filename, prior_filename, logZ,
                       logZe);
        }
...
/**********************************************************************/
void compute_lLikelihood(double *likelihood, const double complex *__restrict mod_vis, double *__restrict res, double *__restrict mod_obs, double *chi2visamp,
                         double *chi2v2, double *chi2t3amp, double *chi2t4amp, double *chi2visphi, double *chi2t3phi, double *chi2t4phi, const int nwavr)
{
...
void vis_to_obs(const double complex *__restrict mod_vis, double *__restrict mod_obs, const int nwavr)
{
  ...
  for (i = 0; i < nt4; ++i)
    {
      modt4 = mod_vis[t4in1[i]] * mod_vis[t4in2[i]] / (mod_vis[t4in3[i]] * conj(mod_vis[t4in4[i]]));
...
double get_flat_chi2(bool benchmark, const int nwavr)
{
  ...
  for (i = 0; i < nbench; ++i)
    compute_lLikelihood(&flat_chi2, mod_vis, res, mod_obs, &dummy1, &dummy2, &dummy3, &dummy4, &dummy5, &dummy6, &dummy7, nwavr);
...
/***********************************/
int writeasfits(const char *file, double *image, int nwavr, long depth, long min_elt, double chi2, double chi2visamp, double chi2v2, double chi2t3amp,
                double chi2t4amp, double chi2visphi, double chi2t3phi, double chi2t4phi, double temperature, long nelems, double *regpar, double *regval,
                long niter, unsigned short axis_len, double ndf, double tmin, double chi2_temp, double chi2_target, double mas_pixel, int nchains, double logZ,
                double logZ_err, char *init_filename, char *prior_filename, double *params, double *params_std)
{
  int status = 0;
  int i, j, w;
  fitsfile *fptr; /* pointer to the FITS file, defined in fitsio.h */
                      ...
                      char *prior_filename, double logZ, double logZe)
{
  ...
  writeasfits(file_basename, image, nwavr, depth, niter - burn_in_times[0] - 1, chi2 / ndf, chi2visamp, chi2v2, chi2t3amp, chi2t4amp, chi2visphi, chi2t3phi,
              chi2t4phi, -1, nelements, reg_param, reg_value, niter, axis_len, ndf, tmin, chi2_temp, chi2_target, mas_pixel, nchains, logZ, logZe,
              init_filename, prior_filename, params, params_std);
}
          ...
          if (nchains_eff < 2)
            {
              ...
              mcmc_writeoutput(data_filename, &images_mean[t * nwavr * axis_len * axis_len], nchains, niter - burn_in_times[t], &burn_in_times[t], depth,
                               nelements, axis_len, xtransform, ytransform, saved_x, saved_y, saved_params, niter, nwavr, final_params, final_params_std,
                               reg_param, final_reg_value, prior_image, initial_x, initial_y, centroid_image_x, centroid_image_y, fov, cent_mult, ndf, tmin,
                               chi2_temp, chi2_target, mas_pixel, init_filename, prior_filename, logZ, logZe);
            }
      ...
      for (t = 0; t < nchains_eff; ++t)
        {
          ...
          mcmc_writeoutput(data_filename, &images_median[t * nwavr * axis_len * axis_len], nchains, niter - burn_in_times[t], &burn_in_times[t], depth,
                           nelements, axis_len, xtransform, ytransform, saved_x, saved_y, saved_params, niter, nwavr, final_params, final_params_std, reg_param,
                           final_reg_value, prior_image, initial_x, initial_y, centroid_image_x, centroid_image_y, fov, cent_mult, ndf, tmin, chi2_temp,
                           chi2_target, mas_pixel, init_filename, prior_filename, logZ, logZe);
        }
    }
      ...
      if (reg_param[REG_CENTERING] > 0)
        {
          ...
          for (i = 0; i < nelements; ++i)
            reg_value[w * NREGULS + REG_CENTERING] +=
                cent_change(w, centroid_image_x, centroid_image_y, initial_x[i], initial_y[i], (axis_len) / 2.0, (axis_len) / 2.0, axis_len, fov, cent_mult);
        }
  ...
  for (i = 2; i < *argc; ++i)
    {
      ...
      if ((strcmp(argv[i], "-h") == 0) || (strcmp(argv[i], "-help") == 0) || (strcmp(argv[i], "--help") == 0))
      ...
      else if (strcmp(argv[i], "-not4amp") == 0)
        {
          *use_t4amp = FALSE; // disable the t4amp
        }
      else if (strcmp(argv[i], "-not4phi") == 0)
        {
          *use_t4phi = FALSE; // disable the t4phi
        }
      else if (strcmp(argv[i], "-not4") == 0)
        {
          *use_t4amp = FALSE; // disable the t4amp
          *use_t4phi = FALSE; // disable the t4phi
        }
      ...
      else if (*argc > i + 1) /* Now the options with arguments*/
        {
          ...
          else if (strcmp(argv[i], "-t4amps") == 0)
            {
              sscanf(argv[i + 1], "%lf", t4amps);
            }
          else if (strcmp(argv[i], "-t4ampa") == 0)
            {
              sscanf(argv[i + 1], "%lf", t4ampa);
            }
          else if (strcmp(argv[i], "-t4phia") == 0)
            {
              sscanf(argv[i + 1], "%lf", t4phia);
            }
          else if (strcmp(argv[i], "-t4phis") == 0)
            {
              sscanf(argv[i + 1], "%lf", t4phis);
            }
...
void print_diagnostics(int iChain, long current_iter, long nvis, long nv2, long nt3, long nt4, long nvisamp, long nt3amp, long nt4amp, long nvisphi,
                       long nt3phi, long nt4phi, double chi2visamp, double chi2v2, double chi2t3amp, double chi2t4amp, double chi2visphi, double chi2t3phi,
                       double chi2t4phi, double lPosterior, double lPrior, double lLikelihood, const double *reg_param, const double *reg_value,
                       const double *centroid_image_x, const double *centroid_image_y, long nelements, int nwavr, long niter, const double *temperature,
  ...
  for (w = 0; w < nwavr; ++w) // diagnostics
    {
      ...
      if (nt4amp > 0)
        diagnostics_used += snprintf(diagnostics + diagnostics_used, maxlength - diagnostics_used, TEXT_COLOR_BLUE "T4A:%5.2f " TEXT_COLOR_BLACK, chi2t4amp);
      if (nt4phi > 0)
        diagnostics_used += snprintf(diagnostics + diagnostics_used, maxlength - diagnostics_used, TEXT_COLOR_GREEN "T4P:%5.2f " TEXT_COLOR_BLACK, chi2t4phi);
