...
long nuv;
char *oifits_file;
long nvis, nv2, nt3, nt3phi, nt3amp, nt3amp_orphans, nt3phi_orphans, nvisamp,
		nvisphi, nvisamp_orphans, nvisphi_orphans;
...
int main(int argc, char** argv)
{
	...
	double tmin = DEFAULT_TMIN, chi2_temp = TARGET_SCALED_CHI2, chi2_target =
			0.0;
	...
	if (read_commandline(&argc, argv, &benchmark, &use_v2, &use_t3amp,
			&use_t3phi, &use_visamp, &use_visphi, &use_diffvis, &use_threadfits,
			&use_bandwidthsmearing, &minimization_engine, &dumpchain,
			&mas_pixel, &axis_len, &depth, &niter, &nelements, &f_anywhere,
			&f_copycat, &nthreads, &tempschedc, &fov, &chi2_temp, &chi2_target,
			&tmin, &prob_auto, &uvtol, &output_filename[0], &init_filename[0],
			&prior_filename[0], &v2s, &v2a, &t3amps, &t3ampa, &t3phia, &t3phis,
			&visamps, &visampa, &visphis, &visphia, &fluxs, &cvfwhm, reg_param,
			init_params, wavmin, wavmax) == FALSE)
		return 0;
	...
	{
		printf(
				"Command line -- Parallel tempering requested but requested number of threads is %d\n",
				nthreads);
		printf(
				"Command line -- Please restart SQUEEZE and set the number of threads N using \"-threads N\"\n");
		return 0;
	}
	...
	if (extract_oifits(argv[1], use_v2, use_t3amp, use_t3phi, use_visamp,
			use_visphi, v2a, v2s, t3ampa, t3amps, t3phia, t3phis, visampa,
			visamps, visphia, visphis, fluxs, cvfwhm, uvtol, wavmin, wavmax,
			timemin, timemax))
	{
	...
	if (nelements == 0)
	{
		nelements = 2.
				* ceil(
						axis_len
								* pow(nv2 + nvisamp + nvisphi + nt3amp + nt3phi,
										0.333));
		if (nelements < 200)
			nelements = 200;
	}
	...
	if (init_filename[0] != 0)
	{
		...
		if (!strcmp(init_filename, "random"))
		{
			...
			{
				initial_x[i] = RngStream_RandInt(initrng, 0, 2147483647)
						% axis_len;
				initial_y[i] = RngStream_RandInt(initrng, 0, 2147483647)
						% axis_len;
			}
		...
		else if (!strcmp(init_filename, "randomthr"))
		{
			...
			for (j = 0; j < nthreads; j++)
			{
				for (i = 0; i < nelements; i++)
				{
					initial_x[j * nelements + i] = RngStream_RandInt(initrng, 0,
							2147483647) % axis_len;
					initial_y[j * nelements + i] = RngStream_RandInt(initrng, 0,
							2147483647) % axis_len;
				}
			}
			RngStream_DeleteStream(&initrng);
		...
		else
		{
			...
			if (fits_read_keys_lng(fptr, "NAXIS", 1, 3, in_naxes, &nfound,
					&status))
				printerror(status);
			...
			printf(
					"Initial image -- reading %ld x %ld pixels x %ld channel(s)\n",
					in_naxes[0], in_naxes[1], k > 2 ? in_naxes[2] : 1);

			if (in_naxes[0] != in_naxes[1])
			{
				printf(
						"Initial image -- input fits file must have a square array.");
				getchar();
			}
			...
			if (fits_read_key_dbl(fptr, "SCALE", &in_mas_pixel, dummy_char,
					&status))
			{
				printf(
						"Initial image -- no SCALE keyword in initial FITS image\n");
				printf(
						"Initial image -- pixel scale assumed to be same as current image\n");
			...
			else
			{
				...
				{
					printf(
							"Initial image -- WARNING init image scale: %lf current %lf\n",
							in_mas_pixel, mas_pixel);
					getchar();
				}
			}
			...
			if (nparams > 0)
			{
				...
				for (i = 0; i < nparams; i++)
				{
					...
					if (!fits_read_key_dbl(fptr, param_string, &init_params[i],
							dummy_char, &status))
					{
						status = 0;
						printf(
								"Initial image -- Could not find MNPARAM%1ld in header\n",
								i);
					}
				}
				status = 0;
			...
			initial_image = malloc(
					in_naxes[0] * in_naxes[0] * nwavi * sizeof(double));

			if (fits_read_img(fptr, TDOUBLE, 1,
					in_naxes[0] * in_naxes[0] * nwavi, &nullval, initial_image,
					&dummy_int, &status))
				printerror(status);
			...
			for (w = 0; w < nwavi; w++)
			{
				...
				for (j = 0; j < in_naxes[0]; j++)
					for (i = 0; i < in_naxes[0]; i++)
						ftot += initial_image[i + j * in_naxes[0]
								+ w * in_naxes[0] * in_naxes[0]];
				printf(
						"Initial image -- checking total flux : %lf in channel %ld\n",
						ftot, w);
				...
				if (fabs((ftot - 1.0)) > 1e-4)
				{
					...
					for (j = 0; j < in_naxes[0]; j++)
						for (i = 0; i < in_naxes[0]; i++)
							initial_image[i + j * in_naxes[0]
									+ w * in_naxes[0] * in_naxes[0]] /= ftot;
				}
			}
			...
			for (w = 0; w < nwavi; w++)
			{
				...
				while (dummy_int < nelements)
				{
					...
					else
					{
						...
						dtemp += initial_image[w * in_naxes[0] * in_naxes[0] + i
								+ j * in_naxes[0]] * nelements; /* Add the flux from the next pixel...*/
					}
				}
			}
	...
	if (prior_filename[0] != 0)
	{
		...
		else
		{
			...
			if (fits_read_img(fptr, TDOUBLE, 1,
					nwavp * in_naxes[0] * in_naxes[0], &nullval, prior_image,
					&dummy_int, &status))
				printerror(status);
			...
			for (w = 0; w < nwavp; w++)
				for (i = 0; i < axis_len; i++)
					for (j = 0; j < axis_len; j++)
						if (prior_image[i + axis_len * j
								+ axis_len * axis_len * w] > 0)
							prior_image[i + axis_len * j
									+ axis_len * axis_len * w] = -log(
									prior_image[i + axis_len * j
											+ axis_len * axis_len * w]);
						else
							prior_image[i + axis_len * j
									+ axis_len * axis_len * w] = 1e9;
	...
	* __restrict ytransform = malloc(axis_len * nuv * sizeof(double complex));
	for (k = 0; k < nuv; k++)
	{
		...
		if (use_bandwidthsmearing == TRUE)
		{
			for (j = 0; j < axis_len; j++)
			{
				xtransform[j * nuv + k] = sinc(
						uv_dlambda[k] / uv_lambda[k] * (j - axis_len / 2) * u[k]
								* mas_pixel / MAS_RAD)
						* cexp(
								I * (j - axis_len / 2) * 2.0 * M_PI * u[k]
										* mas_pixel / MAS_RAD);
				ytransform[j * nuv + k] = sinc(
						uv_dlambda[k] / uv_lambda[k] * (j - axis_len / 2) * v[k]
								* mas_pixel / MAS_RAD)
						* cexp(
								I * (j - axis_len / 2) * 2.0 * M_PI * -v[k]
										* mas_pixel / MAS_RAD);
			}
		}
		else
		{
			for (j = 0; j < axis_len; j++)
			{
				xtransform[j * nuv + k] = cexp(
						I * (j - axis_len / 2) * 2.0 * M_PI * u[k] * mas_pixel
								/ MAS_RAD);
				ytransform[j * nuv + k] = cexp(
						I * (j - axis_len / 2) * 2.0 * M_PI * -v[k] * mas_pixel
								/ MAS_RAD);
			}
		}
	}
	...
	double *saved_reg_value = calloc(nthreads * niter * NREGULS,
			sizeof(double));
	double *saved_params = calloc(nthreads * niter * nparams, sizeof(double));
	unsigned short *saved_x = calloc(nthreads * niter * nwavr * nelements,
			sizeof(unsigned short));
	unsigned short *saved_y = calloc(nthreads * niter * nwavr * nelements,
			sizeof(unsigned short));
...
#pragma omp parallel private(i,j,k,w) shared(temperature, iThreadtoStorage, iStoragetoThread, iMovedThread, burn_in_times, saved_x, saved_y, saved_lLikelihood, \
		...
		reg_param, prior_image,cent_mult,fov,nparams,xtransform,ytransform, ndf)
	{
		...
		{
			temperature[iThread] = pow(
					((double) nthreads) / (1.0 * (nthreads - iThread)),
					tempschedc); // T[0] = 1, T[nthreads-1] = nthreads^tempschedc
...
#pragma omp barrier
		...
		*dummy_cpointer = NULL;
		...
		unsigned short* element_x = malloc(
				nwavr * nelements * sizeof(unsigned short));
		unsigned short* element_y = malloc(
				nwavr * nelements * sizeof(unsigned short));
		double complex
		...
		*new_param_vis = malloc(nuv * sizeof(double complex));
		...
		double *res = malloc(
				(nv2 + nt3amp + nt3phi + nvisamp + nvisphi) * sizeof(double)); // current residuals
		double *mod_obs = malloc(
				(nv2 + nt3amp + nt3phi + nvisamp + nvisphi) * sizeof(double)); // current observables
		...
		// INITIALIZE IMAGE
		//
		initialize_image(iThread, image, element_x, element_y, initial_x,
				initial_y, axis_len, nwavr, nelements, &init_filename[0]);

		//
		// COMPUTE INITIAL REGULARIZER VALUE
		//
		compute_regularizers(reg_param, reg_value, image, prior_image,
				initial_x, initial_y, element_x, element_y, nwavr, axis_len,
				nelements, cent_xoffset, cent_yoffset, fov, cent_mult);

		//
		// COMPUTE INITIAL VISIBILITIES
		//
		compute_model_visibilities_fromelements(mod_vis, im_vis, param_vis,
				params, fluxratio_image, element_x, element_y, xtransform,
				ytransform, &reg_value[REG_MODELPARAM], nparams, nelements);

		//Compute initial values for prior, likelihood, and posterior
		compute_lLikelihood(&lLikelihood, mod_vis, res, mod_obs, &chi2v2,
				&chi2t3amp, &chi2visamp, &chi2t3phi, &chi2visphi);
		...
		if (minimization_engine == ENGINE_SIMULATED_ANNEALING)
		{
			...
			temperature[iThread] = 2.0 * lLikelihood / ndf
					/ INITIAL_SCALED_CHI2_MULT / chi2_temp;
...
#pragma omp critical(savestate) // not sure if this is really needed
				{
					...
					for (w = 0; w < nwavr; w++)
					{
						for (j = 0; j < nelements; j++)
						{
							...
							saved_x[thread1 * nwavr * nelements * niter
									+ w * nelements + i + j] = element_x[w
									* nelements + j];
							saved_y[thread1 * nwavr * nelements * niter
									+ w * nelements + i + j] = element_y[w
									* nelements + j];
						}

					}

					saved_lLikelihood[thread1 * niter + i / (nwavr * nelements)] =
							lLikelihood;
					saved_lPrior[thread1 * niter + i / (nwavr * nelements)] =
							lPrior;
					saved_lPosterior[thread1 * niter + i / (nwavr * nelements)] =
							lPosterior;
					...
					for (j = 0; j < NREGULS; j++)
						saved_reg_value[thread1 * NREGULS * niter
								+ i / (nwavr * nelements) * NREGULS + j] =
								reg_value[j];
				}

				if ((minimization_engine == ENGINE_PARALLEL_TEMPERING)
						&& (i / (nwavr * nelements) > 0)) // note: prevent swapping states before the second iteration
				{
...
#pragma omp barrier   // synchronize threads needed here to prevent swapping while computing the averages
					...
					{
						for (j = ceil(0.3 * niter); j < i / (nwavr * nelements);
								j++)
						{
							// we will average the likelihood for the current thread temperature
							lLikelihood_expectation[iThread] +=
									saved_lLikelihood[iThreadtoStorage[iThread]
											* niter + j];
						}
						lLikelihood_expectation[iThread] /= (i
								/ (nwavr * nelements) - ceil(0.3 * niter));

						for (j = ceil(0.3 * niter); j < i / (nwavr * nelements);
								j++)
						{
							// we will average the likelihood for the current thread temperature
							lLikelihood_deviation[iThread] +=
									(saved_lLikelihood[iThreadtoStorage[iThread]
											* niter + j]
											- lLikelihood_expectation[iThread])
											* (saved_lLikelihood[iThreadtoStorage[iThread]
						...
						if (i / (nwavr * nelements) > ceil(0.3 * niter) + 1)
							lLikelihood_deviation[iThread] /= (i
									/ (nwavr * nelements) - ceil(0.3 * niter)
									- 1);
...
#pragma omp single
					{
						...
						compute_logZ(temperature, iStoragetoThread,
								lLikelihood_expectation, lLikelihood_deviation,
								nthreads, &logZ, &logZ_err);
						printf(
								"log Z computed by thread: %d logZ: %f +/- %f \n",
...
#pragma omp critical(threadswap)
					{
						...
						if (iThread < (nthreads - 1))
						{
							...
							if ((iMovedThread[thread1] == 0)
									&& (iMovedThread[thread2] == 0))
							{
								if (RngStream_RandU01(rng) < 0.3) // base probability of swapping is 30%
								{

									transition_test =
											(1. / temperature[thread1]
													- 1. / temperature[thread2])
													* (saved_lPosterior[iThreadtoStorage[thread1]
															* niter
															+ i
																	/ (nwavr
																			* nelements)]
															- saved_lPosterior[iThreadtoStorage[thread2]
																	* niter
																	+ i
																			/ (nwavr
																					* nelements)]);

									if (log(RngStream_RandU01(rng))
											< transition_test)
									{
										printf(
												"Swap called from thread %d -- will be swapping threads %d : %f and %d : %f at iteration %ld\n",
												iThread, thread1,
												temperature[thread1], thread2,
												temperature[thread2],
												i / (nwavr * nelements));
										...
										swapd(&temperature[thread1],
												&temperature[thread2]);
										swapi(
												&iStoragetoThread[iThreadtoStorage[thread1]],
												&iStoragetoThread[iThreadtoStorage[thread2]]);
										swapi(&iThreadtoStorage[thread1],
												&iThreadtoStorage[thread2]);
...
#pragma omp barrier   //re-synchronise Threads before continuing
			...
			if ((i % (STEPS_PER_OUTPUT * nwavr * nelements)) == 0)
			{
				if (use_threadfits == TRUE)
					writeasfits(temp_filename, image, 1,
							(iThread) * niter + i / (nelements * nwavr),
				...
				print_diagnostics(iThread, (i / (nwavr * nelements) + 1), nvis,
						nv2, nt3, nt3phi, nt3amp, nvisamp, nvisphi, chi2v2,
						chi2t3amp, chi2t3phi, chi2visphi, chi2visamp,
						lPosterior, lPrior, lLikelihood, reg_param, reg_value,
						cent_xoffset, cent_yoffset, nelements, nwavr, niter,
						temperature, prob_movement, params, stepsize);
			...
			if ((nparams == 0) || (current_elt < nelements)) /* Attempt image movement rather than parametric model movement */
			{
				...
				case STEP_ANYWHERE: /* Move flux anywhere in image */
					...
					// rlong /= axis_len;
					xstep = (RngStream_RandInt(rng, 0, 2147483647) % axis_len)
							- axis_len / 2;
					// rlong = RngStream_RandInt(rng, 0, 2147483647);
					ystep = (RngStream_RandInt(rng, 0, 2147483647) % axis_len)
							- axis_len / 2;
					// rlong /= axis_len;
					break;

				case STEP_COPYCAT: /* Copy the position of another element */
					xstep = element_x[chan * nelements + rlong % nelements]
							- element_x[chan * nelements + current_elt];
					ystep = element_y[chan * nelements + rlong % nelements]
							- element_y[chan * nelements + current_elt];
					rlong = RngStream_RandInt(rng, 0, 2147483647);
					break;
				}
				...
				new_x = (element_x[chan * nelements + current_elt] + xstep
						+ axis_len) % axis_len;
				new_y = (element_y[chan * nelements + current_elt] + ystep
						+ axis_len) % axis_len;
				...
				if (reg_param[REG_ENTROPY] > 0.0)
					new_reg_value[chan * NREGULS + REG_ENTROPY] = reg_value[chan
							* NREGULS + REG_ENTROPY] - entropy(image[old_pos])
							+ entropy(image[old_pos] - 1);
				if (reg_param[REG_PRIORIMAGE] > 0.0)
					new_reg_value[chan * NREGULS + REG_PRIORIMAGE] =
							reg_value[chan * NREGULS + REG_PRIORIMAGE]
									- prior_image[old_pos];
				if (reg_param[REG_DARKENERGY] > 0.0)
					new_reg_value[chan * NREGULS + REG_DARKENERGY] =
							reg_value[chan * NREGULS + REG_DARKENERGY]
									- den_change(image, old_x, old_y,
											DEN_SUBTRACT, axis_len);
				if (reg_param[REG_SPOT] > 0.0)
					reg_value[chan * NREGULS + REG_SPOT] = UDreg(
							&image[chan * axis_len * axis_len], NULL, 0.0,
							axis_len, axis_len) / (double) nelements;
				if (reg_param[REG_TV] > 0.0)
					reg_value[chan * NREGULS + REG_TV] = TV(
							&image[chan * axis_len * axis_len], NULL, 0.0,
							axis_len, axis_len) / (double) nelements;
				if (reg_param[REG_LAP] > 0.0)
					reg_value[chan * NREGULS + REG_LAP] = LAP(
							&image[chan * axis_len * axis_len], NULL, 0.0,
							axis_len, axis_len) / (double) nelements;
				if (reg_param[REG_L0] > 0.0)
					reg_value[chan * NREGULS + REG_L0] = L0(
							&image[chan * axis_len * axis_len], NULL, 0.0,
							axis_len, axis_len) / (double) nelements;
				if (reg_param[REG_TRANSPECL2] > 0.0)
					reg_value[REG_TRANSPECL2] = transpec(nwavr, axis_len, image)
							/ (double) nelements;

				image[old_pos]--;

				if (reg_param[REG_ENTROPY] > 0.0)
					new_reg_value[chan * NREGULS + REG_ENTROPY] += entropy(
							image[new_pos] + 1) - entropy(image[new_pos]);
				if (reg_param[REG_PRIORIMAGE] > 0.0)
					new_reg_value[chan * NREGULS + REG_PRIORIMAGE] +=
							prior_image[new_pos];
				if (reg_param[REG_DARKENERGY] > 0.0)
				...
				if (reg_param[REG_SPOT] > 0.0)
					new_reg_value[chan * NREGULS + REG_SPOT] = UDreg(
							&image[chan * axis_len * axis_len], NULL, 0.0,
							axis_len, axis_len) / (double) nelements;  // before
				if (reg_param[REG_TV] > 0.0)
					new_reg_value[chan * NREGULS + REG_TV] = TV(
							&image[chan * axis_len * axis_len], NULL, 0.0,
							axis_len, axis_len) / (double) nelements;  // before
				if (reg_param[REG_LAP] > 0.0)
					new_reg_value[chan * NREGULS + REG_LAP] = LAP(
							&image[chan * axis_len * axis_len], NULL, 0.0,
							axis_len, axis_len) / (double) nelements;  // before

				if (reg_param[REG_L0] > 0.0)
					new_reg_value[chan * NREGULS + REG_L0] = L0(
							&image[chan * axis_len * axis_len], NULL, 0.0,
							axis_len, axis_len) / (double) nelements;  // before

				if (reg_param[REG_TRANSPECL2] > 0.0)
					new_reg_value[REG_TRANSPECL2] = transpec(nwavr, axis_len,
							image) / (double) nelements;
				...
				if (reg_param[REG_CENTERING] > 0.0)
					new_reg_value[chan * NREGULS + REG_CENTERING] =
							reg_value[chan * NREGULS + REG_CENTERING]
									+ fov
											* cent_change(chan, cent_xoffset,
													cent_yoffset, new_x, new_y,
													old_x, old_y, axis_len, fov,
													cent_mult);
				...
				for (j = 0; j < nuv; j++)
				{
					if (uvwav2chan[j] == chan)
					{
						new_im_vis[j] = im_vis[j]
								+ (xtransform[new_x * nuv + j]
										* ytransform[new_y * nuv + j]
										- xtransform[old_x * nuv + j]
												* ytransform[old_y * nuv + j])
										* fluxratio_image[j]
										/ (double) nelements;
					}
					else
			...
			else /* Attempt parametric model movement -- model visibilities are recomputed from scratch */
			{
				...
				model_vis(new_params, new_param_vis,
						&new_reg_value[REG_MODELPARAM], new_fluxratio_image);
				prob_pmovement[j] *= 1.0 - 1.0 / PARAM_DAMPING_TIME;
				stepsize[j] *= 1.0
						+ (prob_pmovement[j] - TARGET_MPROB)
								/ STEPSIZE_ADJUST_TIME;
				for (j = 0; j < nuv; j++)
				{
					new_im_vis[j] = im_vis[j] * new_fluxratio_image[j]
							/ fluxratio_image[j];
			...
			compute_lLikelihood(&new_lLikelihood, new_mod_vis, res, mod_obs,
					&chi2v2, &chi2t3amp, &chi2visamp, &chi2t3phi, &chi2visphi);
			compute_lPrior(&lPrior, chan, reg_param, reg_value);
			...
			transition_test = (new_lLikelihood - lLikelihood)
					/ temperature[iThread] + new_lPrior - lPrior;
			...
			if ((double) (rlong % 1024) + 0.5 < 1024.0 * exp(-transition_test))
			{
				...
				reg_value[chan * NREGULS + REG_CENTERING] = new_reg_value[chan
						* NREGULS + REG_CENTERING];
				...
				if (current_elt < nelements)
				{
					...
					reg_value[chan * NREGULS + REG_ENTROPY] = new_reg_value[chan
							* NREGULS + REG_ENTROPY];
					reg_value[chan * NREGULS + REG_DARKENERGY] =
							new_reg_value[chan * NREGULS + REG_DARKENERGY];
					reg_value[chan * NREGULS + REG_PRIORIMAGE] =
							new_reg_value[chan * NREGULS + REG_PRIORIMAGE];
					reg_value[chan * NREGULS + REG_TV] = new_reg_value[chan
							* NREGULS + REG_TV];
					reg_value[chan * NREGULS + REG_LAP] = new_reg_value[chan
							* NREGULS + REG_LAP];
					reg_value[chan * NREGULS + REG_L0] = new_reg_value[chan
							* NREGULS + REG_L0];
					reg_value[chan * NREGULS + REG_SPOT] = new_reg_value[chan
							* NREGULS + REG_SPOT];
					reg_value[chan * NREGULS + REG_CENTERING] =
							new_reg_value[chan * NREGULS + REG_CENTERING];
				...
				if (minimization_engine == ENGINE_SIMULATED_ANNEALING)
				{
					...
					temperature[iThread] *= 1.0
							+ 1.0 / temperature[iThread] / TEMP_CHANGE_TIME
									* (2. * lLikelihood / ndf
											- chi2_temp * temperature[iThread])
									* (2. * lLikelihood / ndf - chi2_target)
									* ndf / (2. * lLikelihood);

					if ((temperature[iThread] < tmin)
							|| (2. * lLikelihood / ndf < chi2_target)) // convergence
						if ((i / (nwavr * nelements) + niter / BURN_IN_FRAC)
								< burn_in_times[iThread])
							burn_in_times[iThread] = i / (nwavr * nelements)
									+ niter / BURN_IN_FRAC;
			...
			else
			{
				...
				if (current_elt < nelements) // flux movement was attempted and rejected
				{
					...
					if (reg_param[REG_CENTERING] > 0)
						cent_change(chan, cent_xoffset, cent_yoffset, old_x,
								old_y, new_x, new_y, axis_len, fov, cent_mult);
				}
				else     // parameter movement was attempted and rejected
				{
					// error checking
					if (stepsize[(current_elt - nelements) / PARAMS_PER_ELT]
							== 0)
						printf(
								"Failed parameter movement of 0 stepsize! %ld %6.3lf Iter: %ld P: %6.4lf %6.3lf \n",
								(current_elt - nelements) / PARAMS_PER_ELT,
								2. * (new_lLikelihood - lLikelihood), i,
...
#pragma omp barrier    // Synchronize threads
	...
	if (ctrlcpressed == FALSE)
	{
		...
		//
		double* final_image = malloc(
				nwavr * axis_len * axis_len * sizeof(double));
		double* final_params = malloc(nparams * sizeof(double));
		double* final_params_std = malloc(nparams * sizeof(double));
		...
		if (minimization_engine != ENGINE_PARALLEL_TEMPERING) // CASE = (parallel) simulated annealing
		{
			for (i = 0; i < nthreads; i++)
			{
				for (j = burn_in_times[i]; j < niter; j++)
				{
					...
					if (saved_lLikelihood[iframeburned[nburned]]
							< lowest_lLikelihood)
					{
						lowest_lLikelihood =
								saved_lLikelihood[iframeburned[nburned]];
						lowest_lLikelihood_indx = iframeburned[nburned];
					}
					nburned++;
				}
				printf(
						"Output -- Burn-in time / burned frames for thread %ld: %d / %ld\n",
			...
			{
				printf(
						"Output -- DID NOT REACH BURN IN ! Writing final image based on depth parameter anyway\n");
				printf(
						"Output -- DID NOT REACH BURN IN ! Be careful when interpreting the final image.\n");
				for (i = 0; i < nthreads; i++)
			...
			mcmc_annealing_results(output_filename, final_image, iframeburned,
					nburned, nelements, axis_len, xtransform, ytransform, &chi2,
					saved_x, saved_y, saved_params, niter, nwavr, final_params,
					final_params_std);
		...
		else // (minimization_engine == ENGINE_PARALLEL_TEMPERING)
		{
			...
			mcmc_tempering_results(output_filename, final_image,
					iStoragetoThread[0], nburned, nelements, axis_len,
					xtransform, ytransform, &chi2, saved_x, saved_y,
					saved_params, niter, nwavr);

			double logZ_final = 0, logZ_final_err = 0.;

			compute_logZ(temperature, iStoragetoThread, lLikelihood_expectation,
					lLikelihood_deviation, nthreads, &logZ_final,
					&logZ_final_err);
		...
		if (lowest_lLikelihood < 1e99)
			printf(
					"Output -- Best single-frame chi2: %f obtained at iteration: %ld thread: %ld.\n",
					2.0 * lowest_lLikelihood / ndf,
					lowest_lLikelihood_indx % niter,
					lowest_lLikelihood_indx / niter);

		if (dumpchain == TRUE)
			mcmc_fullchain(output_filename, nthreads, niter, nwavr, nelements,
					axis_len, saved_x, saved_y, saved_params, saved_lLikelihood,
					saved_lPrior, saved_lPosterior, temperature,
					iThreadtoStorage);
...
{
	printf(
			"SQUEEZE: an image reconstruction code for optical interferometry \n\n");
	...
	printf(
			"  -d depth       : Number of realizations to average to together for final mean image (default %i).\n",
			DEFAULT_DEPTH);
	printf("  -threads n     : Number of simultaneous threads.\n");
	printf(
			"  -tempschedc c  : Temperature schedule power c for parallel tempering (default = 3).\n");
	printf(
			"  -nobws         : Do not compute bandwidth smearing factors when computing visibilities.\n");

	printf("\n***** OUTPUT SETTINGS ***** \n");
	printf("  -o filename    : Squeeze outputs as a FITS image file.\n");
	printf(
			"  -fullchain     : Output the full MCMC chain into the file output.fullchain .\n");
	printf(
			"  -nothreadfits  : Disable continuous writing of threadxx.fits to accelerate execution.\n");

	printf("\n***** SIMULTANEOUS MODEL FITTING SETTINGS ***** \n");
	printf("  -P p0 p1...    : Initial parameter input.\n");
	printf(
			"  -S s0 s1...    : Initial parameter step sizes (NB: must come after -P option).\n");
	...
	printf(
			"         fs_0     : the stellar flux fraction at the reference wavelength \n");
	printf("         diam     : diameter of the uniform disc\n");
	printf(
			"         d_ind    : the flux power law index for the environment \n");

	printf("\n***** OIFITS IMPORT SETTINGS ***** \n");

	printf(
			"  -wavchan         : Define polychromatic channels for reconstruction.\n");
	printf(
			"                  Usage  : -wavchan 0 wavmin_0 wavmax_0 1 wavmin_1 wavmax_1 ...\n");
	printf(
			"                  Then the wavelength channel i will have points with wavmin_i <= lambda < wavmax_i \n");
	printf(
			"                  Example: -wavchan 0 1.2e-6 1.35e-6 1 1.35e-6 1.43e-6 2 1.6e-6 1.8e-6\n\n");

	printf(
			"  -diffvis      : Switch forcing VIS tables to be treated as differential visibilities, not complex visibilities.\n");
	...
	printf(
			"  -not3phi      : Switch disabling bispectrum phases (closure phases).\n");
	printf("  -nov2         : Switch disabling all powerspectrum data.\n\n");
	printf(
			"  -visamps mult : Scaling factor for visibility amplitude errors.\n");
	printf(
			"  -visampa add  : Additive error to visibility amplitude errors.\n");
	printf(
			"  -visphis mult : Scaling factor for visibility phase/differential phase errors.\n");
	printf(
			"  -visphia add  : Additive error to visibility phase/differential phase errors.\n");
	printf("  -v2s mult     : Scaling factor for visibility-squared errors.\n");
	printf("  -v2a add      : Additive error to visibility-squared errors.\n");
	printf(
			"  -t3amps mult  : Scaling factor for bispectrum amplitude errors.\n");
	printf(
			"  -t3ampa add   : Additive error to bispectrum amplitude errors.\n");
	printf(
			"  -t3phis mult  : Scaling factor for bispectrum amplitude errors.\n");
	printf(
			"  -t3phia add   : Additive error to bispectrum amplitude errors.\n");
	printf(
			"  -fs mult      : Flux scaling factor (zero-baseline visibility intercept).\n");
	printf(
			"  -cv fwhm      : Convolve data and fits by gaussian FWHM mas (default 0.0).\n");
	printf(
			"  -uvtol tol    : Consider all uv points to be the same within tolerance uvtol.\n");
	...
	printf(
			"  -ts param     : Transpectral L2 regularization for polychromatic reconstructions.\n");
	printf("  -fv param     : Field of view regularizer.\n");
	printf(
			"  -p file.fits  : Prior image. Log of this is the regularization.\n");
	printf("  -ps param     : Prior image regularization multiplier.\n");
	printf("  -i file.fits  : Initial image will be read from a FITS file.\n");
	printf(
			"  -i random     : Initial image will be random, and common to all thread/wavelengths.\n");
	printf(
			"  -i randomthr  : Initial image will be random, with a different image for each thread.\n");

	printf("\n***** CONVERGENCE SETTINGS ***** \n");
	printf("  -tm tmin      : Mininmum temperature (default 1.0 for MCMC).\n");
	printf(
			"  -pa prob      : Adjust temperature to achieve given prob for simulated annealing (default = -1, unused).\n");
	printf(
			"  -ct chi2_t    : Chi-squared / Temperature for sim annealing (default 4.0).\n");
	printf(
			"  -fc target    : Attempt to fix chi-squared to this target (default 0.0).\n");

	printf(
			"  -f_copy       : Fraction of steps that attempt a copycat move.\n");
	printf(
			"  -f_any        : Fraction of step that attempt to move anywhere.\n");
...
double residuals_to_chi2(const double *res, double *chi2v2, double *chi2t3amp,
		double *chi2visamp, double *chi2t3phi, double *chi2visphi)
{
	long i;
...
double get_flat_chi2(bool benchmark)
{
	...
	*mod_vis = malloc(nuv * sizeof(double complex));
	...
	for (i = 0; i < nbench; i++)
		compute_lLikelihood(&flat_chi2, mod_vis, res, mod_obs, &dummy1, &dummy2,
				&dummy3, &dummy4, &dummy5);

	endTime = (double) clock() / CLOCKS_PER_SEC;
	if (benchmark == TRUE)
		printf(
				"Reconst setup -- %ld iterations in %f seconds = %f it/sec for this datafile\n",
...
/***********************************/
int writeasfits(char *file, double *image, long depth, long min_elt,
		...
		char *init_filename, char *prior_filename, const double* final_params,
		const double* final_params_std)
{
	int status = 0;
	int i, j, w;
	fitsfile *fptr; /* pointer to the FITS file, defined in fitsio.h */
	...
	if (fits_write_img(fptr, TDOUBLE, fpixel, naxes[0] * naxes[1] * naxes[2],
			image, &status))
		printerror(status);

	/* write another optional keyword to the header */
	/* Note that the ADDRESS of the value is passed in the routine */
	if (fits_write_key_dbl(fptr, "SCALE", mas_pixel, 3,
			"Milli-arcsecs per pixel", &status))
		printerror(status);

	/* write WCS keywords */
	if (fits_write_key_dbl(fptr, "CDELT1", -mas_pixel, 3,
			"Milli-arcsecs per pixel", &status))
		printerror(status);
	if (fits_write_key_dbl(fptr, "CDELT2", mas_pixel, 3,
			"Milli-arcsecs per pixel", &status))
		printerror(status);
	if (fits_write_key_dbl(fptr, "CRVAL1", 0.0, 3, "X-coordinate of ref pixel",
			&status))
		printerror(status);
	if (fits_write_key_dbl(fptr, "CRVAL2", 0.0, 3, "Y-coordinate of ref pixel",
			&status))
		printerror(status);
	if (fits_write_key_lng(fptr, "CRPIX1", naxes[0] / 2, "Ref pixel in X",
			&status))
		printerror(status);
	if (fits_write_key_lng(fptr, "CRPIX2", naxes[1] / 2, "Ref pixel in Y",
			&status))
		printerror(status);
	if (fits_write_key_str(fptr, "CTYPE1", "RA", "Name of X-coordinate",
			&status))
		printerror(status);
	if (fits_write_key_str(fptr, "CTYPE2", "DEC", "Name of Y-coordinate",
			&status))
		printerror(status);
	...
	if (regpar != NULL)
	{
		...
		for (i = 0; i < NREGULS; i++)
		{
			...
			fits_write_key_dbl(fptr, param_string, regpar[i], 3,
	...
	if (temperature >= 0)
	{
		if (fits_update_key(fptr, TDOUBLE, "TEMPER", &temperature,
				"Algorithm Temperature", &status))
			printerror(status);
	}

	if (fits_update_key(fptr, TDOUBLE, "CHISQR", &chi2,
			"Chi-squared per degree of freedom", &status))
		printerror(status);
	if (fits_update_key(fptr, TDOUBLE, "TMIN", &tmin,
			"Setting for minimum temperature", &status))
		printerror(status);
	if (fits_update_key(fptr, TDOUBLE, "CHISQRT", &chi2_temp,
			"Setting for chi-squared divided by temperature (convergence rate)",
			&status))
		printerror(status);
	if (fits_update_key(fptr, TDOUBLE, "CHISQRTG", &chi2_target,
			"Setting for target chi-squared", &status))
		printerror(status);
	if (fits_update_key(fptr, TDOUBLE, "LOGZ", &logZ, "Marginal likelihood",
			&status))
		printerror(status);
	if (fits_update_key(fptr, TDOUBLE, "LOGZE", &logZ_err,
			"Error on Marginal likelihood", &status))
		printerror(status);
	if (fits_update_key(fptr, TSTRING, "INITIM", init_filename,
			"Initialization image.", &status))
		printerror(status);
	if (fits_update_key(fptr, TSTRING, "PRIORIM", prior_filename,
			"Prior image.", &status))
		printerror(status);
	if (fits_update_key(fptr, TSTRING, "OIFITS", oifits_file,
			"Input OIFITS file.", &status))
		printerror(status);
	if (fits_update_key(fptr, TLONG, "NITER", &niter,
			"Number of iterations per thread per element.", &status))
		printerror(status);
	if (fits_update_key(fptr, TINT, "nwavR", &nwavr,
			"Number of spectral channels.", &status))
		printerror(status);
	if (fits_update_key(fptr, TLONG, "DEPTH", &depth,
			"Number of realisations in image", &status))
		printerror(status);
	if (fits_update_key(fptr, TLONG, "ELEMENTS", &nelems,
			"Number of elements per realisation", &status))
		printerror(status);
	if (fits_update_key(fptr, TINT, "NTHREADS", &nthreads, "Number of threads",
			&status))
		printerror(status);
	if (fits_update_key(fptr, TDOUBLE, "NDF", &ndf,
			"Number of degrees of freedom", &status))
		printerror(status);
	if (fits_update_key(fptr, TLONG, "NV2", &nv2, "Number of V2", &status))
		printerror(status);
	if (fits_update_key(fptr, TLONG, "NT3AMP", &nt3amp, "Number of T3AMP",
			&status))
		printerror(status);
	if (fits_update_key(fptr, TLONG, "NT3PHI", &nt3phi, "Number of T3PHI",
			&status))
		printerror(status);
	if (fits_update_key(fptr, TLONG, "NVISAMP", &nvisamp, "Number of VISAMP",
			&status))
		printerror(status);
	if (fits_update_key(fptr, TLONG, "NVISPHI", &nvisphi, "Number of VISPHI",
			&status))
		printerror(status);
...
void compute_logZ(const double* temperature,
		const unsigned short* iStoragetoThread,
		const double* lLikelihood_expectation,
		const double *lLikelihood_deviation, int nthreads, double* logZ,
		double* logZ_err)
{
	long j;
	*logZ = 0;
	*logZ_err = (1. / temperature[iStoragetoThread[1]]
			- 1. / temperature[iStoragetoThread[0]])
			* (1. / temperature[iStoragetoThread[1]]
					- 1. / temperature[iStoragetoThread[0]])
			* lLikelihood_deviation[iStoragetoThread[0]]
			+ (1. / temperature[iStoragetoThread[nthreads - 1]]
					- 1. / temperature[iStoragetoThread[nthreads - 2]])
					* (1. / temperature[iStoragetoThread[nthreads - 1]]
							- 1. / temperature[iStoragetoThread[nthreads - 2]])
	...
	for (j = 0; j < (nthreads - 1); j++)
	{
		...
		*logZ += (1. / temperature[iStoragetoThread[j + 1]]
				- 1. / temperature[iStoragetoThread[j]])
				* (lLikelihood_expectation[iStoragetoThread[j]]
						+ lLikelihood_expectation[iStoragetoThread[j + 1]]);
	}
	...
	*logZ *= 0.5;
	...
	{
		*logZ_err += (1. / temperature[iStoragetoThread[j + 1]]
				- 1. / temperature[iStoragetoThread[j - 1]])
				* (1. / temperature[iStoragetoThread[j + 1]]
						- 1. / temperature[iStoragetoThread[j - 1]])
...
//
void mcmc_fullchain(char *file, long nthreads, long niter, int nwavr,
		long nelements, unsigned short axis_len, unsigned short* saved_x,
		unsigned short* saved_y, double* saved_params,
		double* saved_lLikelihood, double* saved_lPrior,
		double* saved_lPosterior, double* temperature,
		unsigned short* iThreadtoStorage)
{
	int i, n, w, t;
	...
	for (t = 0; t < (nthreads / 3); t++)
	{
		fprintf(pFile2, "%lf , %lf, %lf\n", temperature[3 * t],
				temperature[3 * t + 1], temperature[3 * t + 2]);
	}
	...
	for (t = 0; t < nthreads; t++)
	{
		...
		{
			fprintf(pFile2, "%lf , %lf , %lf\n",
					saved_lLikelihood[iThreadtoStorage[t] * niter + n],
					saved_lPrior[iThreadtoStorage[t] * niter + n],
					saved_lPosterior[iThreadtoStorage[t] * niter + n]);
		}
	}
	...
	double *images = calloc(axis_len * axis_len * nwavr * niter * nthreads,
			sizeof(double));
	if (images == NULL)
	{
	...
	else
	{
		...
		for (t = 0; t < nthreads; t++)
		{
			for (n = 0; n < niter; n++)
			{
				for (w = 0; w < nwavr; w++)
				{
					for (i = 0; i < nelements; i++)
					{
						images[t * niter * axis_len * axis_len * nwavr
								+ n * axis_len * axis_len * nwavr
								+ w * axis_len * axis_len
								+ saved_x[t * nwavr * nelements * niter
										+ n * nwavr * nelements + w * nelements
										+ i]
								+ axis_len
										* saved_y[t * nwavr * nelements * niter
												+ n * nwavr * nelements
												+ w * nelements + i]] += 1;
					}
				}
		...
		if (fits_create_img(fptr, bitpix, naxis, naxes, &status))
			printerror(status);
		if (fits_write_img(fptr, TDOUBLE, fpixel,
				naxes[0] * naxes[1] * naxes[2] * naxes[3] * naxes[4], images,
				&status))
			printerror(status);
		if (fits_update_key(fptr, TLONG, "NITER", &niter,
				"Number of iterations per thread per element.", &status))
			printerror(status);
		if (fits_update_key(fptr, TINT, "NTHREADS", &nthreads,
				"Number of threads", &status))
			printerror(status);
		if (fits_update_key(fptr, TINT, "nwavR", &nwavr,
				"Number of spectral channels.", &status))
			printerror(status);
		if (fits_update_key(fptr, TLONG, "ELEMENTS", &nelements,
				"Number of elements per realisation", &status))
			printerror(status);
...
void mcmc_annealing_results(char *file, double *image, long *iframeburned, long depth, long nelements, unsigned short axis_len,
		...
		unsigned short *saved_x, unsigned short *saved_y, double *saved_params, long niter, int nwavr, double* final_params, double* final_params_std)
...
// the depth input should be the actual available depth, not the requested one
{
	...
	//
	// Compute annealed parametric model and error
	//
	if(nparams > 0)
	{
	...
	compute_model_visibilities_fromimage(mod_vis, im_vis, param_vis, &final_params[0], final_fluxratio_image, image, xtransform, ytransform, &lPriorModel, nparams, nelements, axis_len);
...
void compute_regularizers(double *reg_param, double *reg_value, double* image,
		double* prior_image, unsigned short* initial_x,
		unsigned short* initial_y, unsigned short* element_x,
		unsigned short* element_y, int nwavr, unsigned short axis_len,
		long nelements, double* cent_xoffset, double* cent_yoffset, double fov,
		double cent_mult)
{
	long w, i;
	for (w = 0; w < nwavr; w++)
	{
		if (reg_param[REG_DARKENERGY] > 0)
			reg_value[w * NREGULS + REG_DARKENERGY] = den_full(
					&image[w * axis_len * axis_len], NULL, 0.0, axis_len,
					axis_len);

		if (reg_param[REG_ENTROPY] > 0.0)
			reg_value[w * NREGULS + REG_ENTROPY] = entropy_full(
					&image[w * axis_len * axis_len], NULL, 0.0, axis_len,
					axis_len);

		if (reg_param[REG_SPOT] > 0.0)
			reg_value[w * NREGULS + REG_SPOT] = UDreg(
					&image[w * axis_len * axis_len], NULL, 0.0, axis_len,
					axis_len) / (double) nelements;

		if (reg_param[REG_TV] > 0.0)
			reg_value[w * NREGULS + REG_TV] = TV(
					&image[w * axis_len * axis_len], NULL, 0.0, axis_len,
					axis_len) / (double) nelements;

		if (reg_param[REG_LAP] > 0.0)
			reg_value[w * NREGULS + REG_LAP] = LAP(
					&image[w * axis_len * axis_len], NULL, 0.0, axis_len,
					axis_len) / (double) nelements;

		if (reg_param[REG_L0] > 0.0)
			reg_value[w * NREGULS + REG_L0] = L0(
					&image[w * axis_len * axis_len], NULL, 0.0, axis_len,
					axis_len) / (double) nelements;
		...
		if (reg_param[REG_PRIORIMAGE] > 0)
			for (i = 0; i < nelements; i++)
				reg_value[w * NREGULS + REG_PRIORIMAGE] +=
						prior_image[element_y[w * nelements + i] * axis_len
								+ element_x[w * nelements + i]];

		reg_value[w * NREGULS + REG_CENTERING] = 0;
		if (reg_param[REG_CENTERING] > 0)
			for (i = 0; i < nelements; i++)
				reg_value[w * NREGULS + REG_CENTERING] += cent_change(w,
						cent_xoffset, cent_yoffset, initial_x[i], initial_y[i],
						axis_len / 2, axis_len / 2, axis_len, fov, cent_mult);
...
void initialize_image(int iThread, double* image, unsigned short* element_x,
		unsigned short* element_y, unsigned short* initial_x,
		unsigned short* initial_y, unsigned short axis_len, int nwavr,
		long nelements, char* init_filename)
{
	long i, k, w;
	for (i = 0; i < nwavr * axis_len * axis_len; i++)
		image[i] = 0;
	...
	for (w = 0; w < nwavr; w++)
	{
		for (i = 0; i < nelements; i++)
		{
			...
			image[w * axis_len * axis_len + initial_y[i] * axis_len
					+ initial_x[i]]++;
		}
	}
}

bool read_commandline(int* argc, char** argv, bool* benchmark, bool* use_v2,
		bool* use_t3amp, bool* use_t3phi, bool* use_visamp, bool* use_visphi,
		bool* use_diffvis, bool* use_threadfits, bool* use_bandwidthsmearing,
		int* minimization_engine, bool* dumpchain, double* mas_pixel,
		unsigned short* axis_len, long* depth, long* niter, long* nelements,
		double* f_anywhere, double* f_copycat, int *nthreads,
		double* tempschedc, double* fov, double* chi2_temp, double* chi2_target,
		double* tmin, double* prob_auto, double* uvtol, char* output_filename,
		char* init_filename, char* prior_filename, double* v2s, double* v2a,
		double* t3amps, double* t3ampa, double* t3phia, double* t3phis,
		double* visamps, double* visampa, double* visphis, double* visphia,
		double* fluxs, double* cvfwhm, double* reg_param, double* init_param,
		double* wavmin, double* wavmax)
{
	long i, j, k;
	...
	for (i = 2; i < *argc; i++)
	{
		...
		else if (*argc > i + 1) /* Now the options with arguments*/
		{
			...
			else if (strcmp(argv[i], "-wavchan") == 0)
			{
				...
				printf(
						"Command line  -- set to polychromatic reconstruction\n");
				...
				for (j = 0; j < nwavr; j++)
				{
					...
					{
						printf(
								"Command line  -- Misformed channel information \n");
						getchar();
						break;
					}
					else
					{
						printf(
								"Command line  -- channel %ld = %lg m to\t %lg m\n",
								k, wavmin[k], wavmax[k]);
					}
				}
				i += wavchaninfo - 1;
			...
			{
				printf(
						"Command line  -- invalid option %s. Type squeeze -h for help.\n",
						argv[i]);
				return FALSE;
			}
			i++;
		...
		{
			printf(
					"Command line  -- invalid option. Type squeeze -h for help.\n");
			return FALSE;
		}
...
void print_diagnostics(int iThread, long current_iter, long nvis, long nv2,
		long nt3, long nt3phi, long nt3amp, long nvisamp, long nvisphi,
		double chi2v2, double chi2t3amp, double chi2t3phi, double chi2visphi,
		double chi2visamp, double lPosterior, double lPrior, double lLikelihood,
		const double* reg_param, const double* reg_value,
		const double* cent_xoffset, const double* cent_yoffset, long nelements,
		int nwavr, long niter, const double* temperature, double prob_movement,
		const double* params, const double* stepsize)
{
	...
	for (w = 0; w < nwavr; w++) // diagnostics
	{
		if (nwavr > 1)
			diagnostics_used =
					snprintf(diagnostics, 250,
							"Thread: %d Chan: %ld lPost:%8.1f lPrior:%8.1f lLike:%9.1f ",
							iThread, w, lPosterior, lPrior, lLikelihood);
		else
			diagnostics_used = snprintf(diagnostics, 250,
					"Thread: %d lPost:%8.1f lPrior:%8.1f lLike:%9.1f ", iThread,
					lPosterior, lPrior, lLikelihood);

		if (nv2 > 0)
			diagnostics_used += snprintf(diagnostics + diagnostics_used,
					250 - diagnostics_used,
		...
		if (nvisamp > 0)
			diagnostics_used += snprintf(diagnostics + diagnostics_used,
					250 - diagnostics_used, "VA:%5.2f ", chi2visamp);
		if (nvisphi > 0)
			diagnostics_used += snprintf(diagnostics + diagnostics_used,
					250 - diagnostics_used, "VP:%5.2f ", chi2visphi);
		if (reg_param[REG_PRIORIMAGE] > 0)
			diagnostics_used += snprintf(diagnostics + diagnostics_used,
					250 - diagnostics_used, "PRI:%5.2f ",
					reg_param[REG_PRIORIMAGE]
							* reg_value[w * NREGULS + REG_PRIORIMAGE]);
		if (reg_param[REG_ENTROPY] > 0)
			diagnostics_used += snprintf(diagnostics + diagnostics_used,
					250 - diagnostics_used, "ENT:%5.2f ",
					reg_param[REG_ENTROPY]
							* reg_value[w * NREGULS + REG_ENTROPY]);
		if (reg_param[REG_DARKENERGY] > 0)
			diagnostics_used += snprintf(diagnostics + diagnostics_used,
					250 - diagnostics_used, "DEN:%5.2f ",
					reg_param[REG_DARKENERGY]
							* reg_value[w * NREGULS + REG_DARKENERGY]);
		if (reg_param[REG_SPOT] > 0)
			diagnostics_used += snprintf(diagnostics + diagnostics_used,
					250 - diagnostics_used, "UD:%5.2f ",
					reg_param[REG_SPOT] * reg_value[w * NREGULS + REG_SPOT]);
		if (reg_param[REG_TV] > 0)
			diagnostics_used += snprintf(diagnostics + diagnostics_used,
					250 - diagnostics_used, "TV:%5.2f ",
					reg_param[REG_TV] * reg_value[w * NREGULS + REG_TV]);
		if (reg_param[REG_LAP] > 0)
			diagnostics_used += snprintf(diagnostics + diagnostics_used,
					250 - diagnostics_used, "LAP:%5.2f ",
					reg_param[REG_LAP] * reg_value[w * NREGULS + REG_LAP]);
		if (reg_param[REG_L0] > 0)
			diagnostics_used += snprintf(diagnostics + diagnostics_used,
					250 - diagnostics_used, "L0:%5.2f ",
					reg_param[REG_L0] * reg_value[w * NREGULS + REG_L0]);
		if (reg_param[REG_TRANSPECL2] > 0)
			diagnostics_used += snprintf(diagnostics + diagnostics_used,
					250 - diagnostics_used, "TS:%5.2f ",
					reg_param[REG_TRANSPECL2] * reg_value[REG_TRANSPECL2]);
		if (reg_param[REG_CENTERING] > 0)
			diagnostics_used += snprintf(diagnostics + diagnostics_used,
					250 - diagnostics_used, "C:%5.1f XY:(%5.2f,%5.2f) ",
					reg_param[REG_CENTERING]
							* reg_value[w * NREGULS + REG_CENTERING],
					cent_xoffset[w] / nelements, cent_yoffset[w] / nelements);

		diagnostics_used += snprintf(diagnostics + diagnostics_used,
				250 - diagnostics_used,
				"E: %5ld MPr: %4.2f T: %5.2f Iter: %4ld of %4ld", nelements,
				prob_movement, temperature[iThread], current_iter, niter);
...
void compute_lPrior(double* lPrior, const long chan, const double* reg_param,
		const double* reg_value)
{
	*lPrior = reg_param[REG_PRIORIMAGE]
			* reg_value[chan * NREGULS + REG_PRIORIMAGE]
			+ reg_param[REG_CENTERING]
					* reg_value[chan * NREGULS + REG_CENTERING]
			+ reg_param[REG_ENTROPY] * reg_value[chan * NREGULS + REG_ENTROPY]
			+ reg_param[REG_MODELPARAM]
					* reg_value[chan * NREGULS + REG_MODELPARAM]
			+ reg_param[REG_DARKENERGY]
					* reg_value[chan * NREGULS + REG_DARKENERGY]
