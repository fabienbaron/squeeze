...
int main(int argc, char** argv)
{
    ...
    if (read_commandline(&argc, argv, &benchmark, &use_v2, &use_t3amp,
                         &use_t3phi, &use_visamp, &use_visphi, &use_diffvis, &use_tempfitswriting,
                         &use_bandwidthsmearing, &minimization_engine, &dumpchain,
                         &mas_pixel, &axis_len, &depth, &niter, &nelements, &f_anywhere,
                         &f_copycat, &nchains, &nthreads, &tempschedc, &fov, &chi2_temp, &chi2_target,
                ...
                for (i = 0; i < axis_len; i++)
                    for (j = 0; j < axis_len; j++)
                        if (prior_image[i + axis_len * j + axis_len * axis_len * w] > 0)
                            prior_image[i + axis_len * j + axis_len * axis_len * w] = -log(
                                        prior_image[i + axis_len * j + axis_len * axis_len * w]);
                        else
    ...
    * __restrict ytransform = malloc(axis_len * nuv * sizeof(double complex));
    ...
    double *saved_reg_value = calloc(nchains * niter * NREGULS,
                                     sizeof(double));
    double *saved_params = calloc(nchains * niter * nparams, sizeof(double));
    unsigned short *saved_x = calloc(nchains * niter * nwavr * nelements,
                                     sizeof(unsigned short));
    unsigned short *saved_y = calloc(nchains * niter * nwavr * nelements,
                                     sizeof(unsigned short));
    ...
    // Start nchains MCMC
    //
    #pragma omp parallel num_threads(nchains) private(i,j,k,w) shared(temperature, iChaintoStorage, iStoragetoChain, iMovedChain, burn_in_times, saved_x, saved_y, saved_lLikelihood, \
    ...
    reg_param, prior_image,cent_mult,fov,nparams,xtransform,ytransform, ndf)
    {
        ...
        int iChain = 0;
            ...
            {
                temperature[iChain] = pow(
                                          ((double) nchains) / (1.0 * (nchains - iChain)),
                                          tempschedc); // T[0] = 1, T[nchains-1] = nchains^tempschedc
            ...
            *dummy_cpointer = NULL;
            ...
            sprintf(temp_filename, "thread%02d", iChain);
            ...
            // INITIALIZE IMAGE
            //
            initialize_image(iChain, image, element_x, element_y, initial_x,
                             initial_y, axis_len, nwavr, nelements, &init_filename[0]);

            //
            // COMPUTE INITIAL REGULARIZER VALUE
            //
            compute_regularizers(reg_param, reg_value, image, prior_image, (double)nelements,
                                 initial_x, initial_y, nwavr, axis_len,
                                 nelements, centroid_image_x, centroid_image_y, fov, cent_mult);
            ...
            if (minimization_engine == ENGINE_SIMULATED_ANNEALING)
            {
                ...
                temperature[iChain] = 2.0 * lLikelihood / ndf
            ...
            for (i = 0; i < niter * nwavr * nelements; i++)
            {
                ...
                {
                    thread1 = iChaintoStorage[iChain];
                    ...
                    #pragma omp critical(savestate) // not sure if this is really needed
                    {
                        ...
                        for (w = 0; w < nwavr; w++)
                        {
                            for (j = 0; j < nelements; j++)
                            {
                                ...
                                // Note: in parallel tempering mode, iChaintoStorage[iChain] tracks which thread correspond to saving slot
                            ...
                            && (i / (nwavr * nelements) > 0)) // note: prevent swapping states before the second iteration
                    {
                        ...
                        if (i / (nwavr * nelements) > ceil(0.3 * niter)) // note: we need to check for burn-in info here instead
                        {
                            ...
                            {
                                // we will average the likelihood for the current thread temperature
                                lLikelihood_deviation[iChain] +=
                                    (saved_lLikelihood[iChaintoStorage[iChain]
                                                       * niter + j]
                                     - lLikelihood_expectation[iChain])
                                    * (saved_lLikelihood[iChaintoStorage[iChain]
                                                         * niter + j]
                                       - lLikelihood_expectation[iChain]);
                            }
                            if (i / (nwavr * nelements) > ceil(0.3 * niter) + 1)
                                lLikelihood_deviation[iChain] /= (i
                        ...
                        #pragma omp single
                        {
                            ...
                            compute_logZ(temperature, iStoragetoChain,
                                         lLikelihood_expectation, lLikelihood_deviation,
                                         nchains, &logZ, &logZ_err);
                            printf(
                        ...
                        fflush(stdout);
                        iMovedChain[thread1] = 0; // set all threads as ready to switch
                        #pragma omp barrier
                        #pragma omp critical(threadswap)
                        {
                            ...
                            //    iStoragetoChain[N] give us the index of the thread that currently has the Nth temperature (in the sorted list of increasing temperatures)
                            ...
                            // 3) we also track which threads have been swapped through iMovedChain, to prevent sequential swapping (0->1 then 1->2)

                            if (iChain < (nchains - 1))
                            {
                                thread1 = iStoragetoChain[iChain];
                                thread2 = iStoragetoChain[iChain + 1];

                                if ((iMovedChain[thread1] == 0)
                                        && (iMovedChain[thread2] == 0))
                                {
                                    if (RngStream_RandU01(rng) < 0.3) // base probability of swapping is 30%
                                    {
                                        ...
                                        transition_test =
                                            ...
                                            * (saved_lPosterior[iChaintoStorage[thread1]
                                                                * niter
                                                                + i
                                                                / (nwavr
                                                                   * nelements)]
                                               - saved_lPosterior[iChaintoStorage[thread2]
                                                                  * niter
                                                                  + i
                                                                  / (nwavr
                                                                     * nelements)]);
                                                ...
                                                < transition_test)
                                        {
                                            ...
                                            swapd(&temperature[thread1],
                                                  &temperature[thread2]);
                                            swapi(
                                                &iStoragetoChain[iChaintoStorage[thread1]],
                                                &iStoragetoChain[iChaintoStorage[thread2]]);
                                            swapi(&iChaintoStorage[thread1],
                                                  &iChaintoStorage[thread2]);

                                            // printf("Swap done at thread %d -- has swapped threads %d : %f and %d : %f at iteration %ld\n",
                                            //         iChain, thread2, temperature[thread2], thread1,  temperature[thread1], i / (nwavr * nelements));

                                            // for(j=0;j<nchains;j++)
                                            //  printf("AFTER Chain %ld \t iChaintoStorage %d \t iStoragetoChain %d \t temp[j] %lf \t temp[iStorage[j]] %lf \t temp[iChain[j] %lf\n", j, iChaintoStorage[j], iStoragetoChain[j], temperature[j], temperature[iStoragetoChain[j]],temperature[iChaintoStorage[j]] );

                                            iMovedChain[thread1] = 1;
                                            iMovedChain[thread2] = 1;
                                        }
                                    }
                                }
                            }
                ...
                if ((i % (STEPS_PER_OUTPUT * nwavr * nelements)) == 0)
                {
                    if (use_tempfitswriting == TRUE)
                        writeasfits(temp_filename, image, 1,
                                    (iChain) * niter + i / (nelements * nwavr),
                                    2.0 * lLikelihood / ndf, temperature[iChain],
                    ...
                    // PRINT DIAGNOSTICS
                    print_diagnostics(iChain, (i / (nwavr * nelements) + 1), nvis,
                                      ...
                                      centroid_image_x, centroid_image_y, nelements, nwavr, niter,
                ...
                if ((nparams == 0) || (current_elt < nelements)) /* Attempt image movement rather than parametric model movement */
                {
                    ...
                    if (reg_param[REG_DARKENERGY] > 0.0)
                        new_reg_value[chan * NREGULS + REG_DARKENERGY] = reg_value[chan * NREGULS + REG_DARKENERGY] - den_change(image, old_x, old_y, DEN_SUBTRACT, axis_len);
                    ...
                    if (reg_param[REG_SPOT] > 0.0)
                        new_reg_value[chan * NREGULS + REG_SPOT] = UDreg(&image[chan * axis_len * axis_len], NULL, 0.0,axis_len, axis_len, (const double) nelements);  // before
                    if (reg_param[REG_TV] > 0.0)
                ...
                transition_test = (new_lLikelihood - lLikelihood)
                                  / temperature[iChain] + new_lPrior - lPrior;
                ...
                if ((double) (rlong % 1024) + 0.5 < 1024.0 * exp(-transition_test))
                {
                    ...
                    if (minimization_engine == ENGINE_SIMULATED_ANNEALING)
                    {
                        ...
                        temperature[iChain] *= 1.0
                                               + 1.0 / temperature[iChain] / TEMP_CHANGE_TIME
                                               * (2. * lLikelihood / ndf
                                                  - chi2_temp * temperature[iChain])
                        ...
                        if ((temperature[iChain] < tmin)
                            ...
                            if ((i / (nwavr * nelements) + niter / BURN_IN_FRAC)
                                    < burn_in_times[iChain])
                                burn_in_times[iChain] = i / (nwavr * nelements)
                                                        + niter / BURN_IN_FRAC;
                ...
                else
                {
                    ...
                    if (current_elt < nelements) // flux movement was attempted and rejected
                    {
                        ...
                        if (reg_param[REG_CENTERING] > 0)
                            cent_change(chan, centroid_image_x, centroid_image_y, old_x,
    ...
    if (ctrlcpressed == FALSE)
{
    ...
    long *iframeburned = malloc(nchains * niter * sizeof(long));
        ...
        if (minimization_engine != ENGINE_PARALLEL_TEMPERING) // CASE = (parallel) simulated annealing
        {
            ...
            mcmc_annealing_results(output_filename, final_image, iframeburned, nburned, nelements, axis_len,
                                   xtransform, ytransform, &chi2, saved_x, saved_y, saved_params, niter, nwavr, final_params, final_params_std,
                                   reg_param, final_reg_value, prior_image, initial_x, initial_y, centroid_image_x, centroid_image_y, fov, cent_mult);
            ...
            writeasfits(output_filename, final_image, k,
                        ...
                        chi2_target, mas_pixel, nchains, 0, 0,	init_filename, prior_filename, final_params, final_params_std);
        ...
        else // (minimization_engine == ENGINE_PARALLEL_TEMPERING)
        {
            ...
            mcmc_tempering_results(output_filename, final_image,
                                   iStoragetoChain[0], nburned, nelements, axis_len,
                                   xtransform, ytransform, &chi2, saved_x, saved_y,
                                   saved_params, niter, nwavr);

            double logZ_final = 0, logZ_final_err = 0.;

            compute_logZ(temperature, iStoragetoChain, lLikelihood_expectation,
                         lLikelihood_deviation, nchains, &logZ_final,
                         &logZ_final_err);
            ...
            writeasfits(output_filename, final_image, k,
                        ...
                        chi2_target, mas_pixel, nchains, logZ_final, logZ_final_err, init_filename, prior_filename, final_params, final_params_std);
        }
        ...
        if (dumpchain == TRUE)
            mcmc_fullchain(output_filename, nchains, niter, nwavr, nelements,
                 ...
                 double* params_std)
{
    ...
    if (fits_update_key(fptr, TLONG, "ELEMENTS", &nelems,
        ...
        printerror(status);
    if (fits_update_key(fptr, TINT, "NCHAINS", &nchains, "Number of chains",
                        &status))
        printerror(status);
...
void compute_logZ(const double* temperature,
                  const unsigned short* iStoragetoChain,
                  const double* lLikelihood_expectation,
                  const double *lLikelihood_deviation, int nchains, double* logZ,
                  double* logZ_err)
{
    long j;
    *logZ = 0;
    *logZ_err = (1. / temperature[iStoragetoChain[1]]
                 - 1. / temperature[iStoragetoChain[0]])
                * (1. / temperature[iStoragetoChain[1]]
                   - 1. / temperature[iStoragetoChain[0]])
                * lLikelihood_deviation[iStoragetoChain[0]]
                + (1. / temperature[iStoragetoChain[nchains - 1]]
                   - 1. / temperature[iStoragetoChain[nchains - 2]])
                * (1. / temperature[iStoragetoChain[nchains - 1]]
                   - 1. / temperature[iStoragetoChain[nchains - 2]])
    ...
    for (j = 0; j < (nchains - 1); j++)
    {
        ...
        *logZ += (1. / temperature[iStoragetoChain[j + 1]]
                  - 1. / temperature[iStoragetoChain[j]])
                 * (lLikelihood_expectation[iStoragetoChain[j]]
                    + lLikelihood_expectation[iStoragetoChain[j + 1]]);
    }
    ...
    *logZ *= 0.5;
    ...
    {
        *logZ_err += (1. / temperature[iStoragetoChain[j + 1]]
                      - 1. / temperature[iStoragetoChain[j - 1]])
                     * (1. / temperature[iStoragetoChain[j + 1]]
                        - 1. / temperature[iStoragetoChain[j - 1]])
                     * lLikelihood_deviation[iStoragetoChain[j]];
    }
...
//
void mcmc_fullchain(char *file, long nchains, long niter, int nwavr,
                    ...
                    unsigned short* iChaintoStorage)
{
    ...
    for (t = 0; t < nchains; t++)
    {
        ...
        {
            fprintf(pFile2, "%lf , %lf , %lf\n",
                    saved_lLikelihood[iChaintoStorage[t] * niter + n],
                    saved_lPrior[iChaintoStorage[t] * niter + n],
    ...
    double *images = calloc(axis_len * axis_len * nwavr * niter * nchains,
                            sizeof(double));
    if (images == NULL)
    {
    ...
    else
    {
        ...
        if (fits_update_key(fptr, TLONG, "NITER", &niter,
                            "Number of iterations per thread per element.", &status))
            printerror(status);
        if (fits_update_key(fptr, TINT, "NCHAINS", &nchains,
                            "Number of chains", &status))
            printerror(status);
        if (fits_update_key(fptr, TINT, "nwavR", &nwavr,
                            "Number of spectral channels.", &status))
            printerror(status);
...
void compute_regularizers(const double *reg_param, double *reg_value, const double* image,
                          const double* prior_image, const double fluxscaling, const unsigned short* initial_x,
                          ...
                          const double cent_mult)
{
    ...
    for (w = 0; w < nwavr; w++)
    {
        ...
        if (reg_param[REG_PRIORIMAGE] > 0)
            reg_value[w * NREGULS + REG_PRIORIMAGE] = reg_prior_image(&image[w * axis_len * axis_len], &prior_image[w*axis_len*axis_len], 0.0, axis_len, axis_len, fluxscaling);

        if (reg_param[REG_CENTERING] > 0)
        {
            reg_value[w * NREGULS + REG_CENTERING] = 0;
            for (i = 0; i < nelements; i++)
                reg_value[w * NREGULS + REG_CENTERING] += cent_change(w, centroid_image_x, centroid_image_y, initial_x[i], initial_y[i],
...
void initialize_image(int iChain, double* image, unsigned short* element_x,
...
bool read_commandline(int* argc, char** argv, bool* benchmark, bool* use_v2,
                      ...
                      bool* use_diffvis, bool* use_tempfitswriting, bool* use_bandwidthsmearing,
                      int* minimization_engine, bool* dumpchain, double* mas_pixel,
                      unsigned short* axis_len, long* depth, long* niter, long* nelements,
                      double* f_anywhere, double* f_copycat, int *nchains, int* nthreads,
...
void print_diagnostics(int iChain, long current_iter, long nvis, long nv2,
                       ...
                       const double* centroid_image_x, const double* centroid_image_y, long nelements,
                       ...
                       const double* params, const double* stepsize)
{
    ...
    for (w = 0; w < nwavr; w++) // diagnostics
    {
        ...
        if (nwavr > 1)
            diagnostics_used = snprintf(diagnostics, 250, "Chain: %d Chan: %ld lPost:%8.1f lPrior:%8.1f lLike:%9.1f ", iChain, w, lPosterior, lPrior, lLikelihood);
        else
        ...
        if (reg_param[REG_CENTERING] > 0)
            diagnostics_used += snprintf(diagnostics + diagnostics_used,
                                         ...
                                         * reg_value[w * NREGULS + REG_CENTERING],
                                         centroid_image_x[w] / nelements, centroid_image_y[w] / nelements);
